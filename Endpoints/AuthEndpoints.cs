...
public static class AuthEndpoints
{
    ...
    CancellationToken cancellationToken = default) // ← quita ICrudService, ya no se necesita aquí
{
    try
    {
        ...
        if (string.IsNullOrWhiteSpace(request.Password))
            return Results.BadRequest(new { success = false, error = "ValidationError", message = "La contraseña es requerida" });

        if (string.IsNullOrWhiteSpace(request.Rol))
            return Results.BadRequest(new { success = false, error = "ValidationError", message = "El rol es requerido" });

        var validRoles = new[] { "cliente", "mayorista", "operador" };
        var rol = request.Rol.ToLowerInvariant();
        if (!validRoles.Contains(rol))
            return Results.BadRequest(new { success = false, error = "ValidationError", message = "Rol inválido. Debe ser: cliente, mayorista u operador" });
        ...
        // ✅ ESTOS FALTABAN — el trigger los leerá de raw_user_meta_data
        if (!string.IsNullOrWhiteSpace(request.Rfc))
            metadata["rfc"] = request.Rfc;

        if (!string.IsNullOrWhiteSpace(request.TelefonoContacto))
            metadata["telefono_contacto"] = request.TelefonoContacto;

        if (!string.IsNullOrWhiteSpace(request.DireccionFiscal))
            metadata["direccion_fiscal"] = request.DireccionFiscal;

        if (!string.IsNullOrWhiteSpace(request.Ubicacion))
            metadata["ubicacion"] = request.Ubicacion;
        ...
        httpClient.DefaultRequestHeaders.Authorization =
            System.Net.Http.Headers.AuthenticationHeaderValue.Parse($"Bearer {supabaseKey}");
        ...
        var response = await httpClient.PostAsync(
            $"{GetAuthUrl(configuration)}/admin/users", content, cancellationToken)
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            return Results.BadRequest(new { success = false, error = "BadRequest", message = errorMsg });
        }

        // ✅ Leer el id del usuario recién creado en Supabase
        var createdUser = JsonSerializer.Deserialize<JsonElement>(responseJson);
        var userId = createdUser.TryGetProperty("id", out var idProp)
            ? idProp.GetString()
            : null;
        ...
        return Results.Ok(new
        {
            ...
            data = new { id = userId, email = request.Email, rol = request.Rol }
        });
    }
